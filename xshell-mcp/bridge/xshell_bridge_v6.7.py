...
def _handle_check():
    return {"success": True, "output": "bridge v6.7 online",
...
def Main():
    ...
    _log("v6.7 STARTED pid=%s ipc=%s cfg=%s" % (
        os.getpid(), IPC_DIR,
        {k: v for k, v in vars(CFG).items() if not k.startswith("_")}
    ))

    _write_resp({
        "success": True, "output": "bridge v6.7 started", "timed_out": False,
        "start_row": 0, "end_row": 0,
        "screen_rows": _total_rows(), "screen_cols": CFG.SCREEN_COLS,
    ...
    _write_resp({
        "success": False, "error": "bridge exiting", "output": "bridge v6.7 stopped",
        "timed_out": False, "start_row": 0, "end_row": 0,
