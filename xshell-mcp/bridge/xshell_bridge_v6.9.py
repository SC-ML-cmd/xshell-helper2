...
- **新增**：命令追加 `2>&1` 将 stderr 重定向到 stdout，
  确保错误信息也能被捕获
- **修复**：WaitForString 替换为 WaitForStrings，增加 timeout 保护，
  超时自动 Ctrl+C，解决网络波动导致 Bridge 永久卡死
"""
...
def _handle_exec(req):
    ...
    timed_out = False
    try:
        found = xsh.Screen.WaitForStrings([marker], timeout_ms)
        timed_out = (found == 0)
    except Exception as e1:
        _log("WaitForStrings[list]: %s" % e1)
        try:
            found = xsh.Screen.WaitForStrings((marker,), timeout_ms)
            timed_out = (found == 0)
        except Exception as e2:
            _log("WaitForStrings[tuple]: %s" % e2)
...
def _handle_send_raw(req):
    ...
    if wait_for:
        try:
            found = xsh.Screen.WaitForStrings([wait_for], timeout_ms)
            timed_out = (found == 0)
        except Exception as e1:
            _log("WaitForStrings[list]: %s" % e1)
            try:
...
def _handle_check():
    return {"success": True, "output": "bridge v6.9 online",
...
def Main():
    ...
    _log("v6.9 STARTED pid=%s ipc=%s cfg=%s" % (
        os.getpid(), IPC_DIR,
        {k: v for k, v in vars(CFG).items() if not k.startswith("_")}
    ))

    _write_resp({
        "success": True, "output": "bridge v6.9 started", "timed_out": False,
        "start_row": 0, "end_row": 0,
        "screen_rows": _total_rows(), "screen_cols": CFG.SCREEN_COLS,
    ...
    _write_resp({
        "success": False, "error": "bridge exiting", "output": "bridge v6.9 stopped",
        "timed_out": False, "start_row": 0, "end_row": 0,
