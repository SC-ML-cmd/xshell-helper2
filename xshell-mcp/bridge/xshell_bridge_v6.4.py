...
- 配置集中管理，方便调试
- 日志带 tid，区分不同窗口/页签
- **修复**：_handle_exec / _handle_send_raw 改用 _total_rows()，
  解决 CurrentRow 不准确导致命令输出丢失的问题
"""
...
def _handle_check():
    return {"success": True, "output": "bridge v6.4 online",
...
def Main():
    ...
    _log("v6.4 STARTED pid=%s ipc=%s cfg=%s" % (
        os.getpid(), IPC_DIR,
        {k: v for k, v in vars(CFG).items() if not k.startswith("_")}
    ))

    _write_resp({
        "success": True, "output": "bridge v6.4 started", "timed_out": False,
        "start_row": 0, "end_row": 0,
        "screen_rows": _total_rows(), "screen_cols": CFG.SCREEN_COLS,
    ...
    _write_resp({
        "success": False, "error": "bridge exiting", "output": "bridge v6.4 stopped",
        "timed_out": False, "start_row": 0, "end_row": 0,
